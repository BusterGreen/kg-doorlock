...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and KGCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
KGCore.Functions.CreateCallback('kg-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

KGCore.Functions.CreateCallback('kg-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('kg-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('kg-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('kg-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('kg-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not KGCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = KGCore.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('kg-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('kg-doorlock:server:removeLockpick', function(type)
	local Player = KGCore.Functions.GetPlayer(source)
...
KGCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('kg-doorlock:client:addNewDoor', source)
end, Config.CommandPermission)

KGCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('kg-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
